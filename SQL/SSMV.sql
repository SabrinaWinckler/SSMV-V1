...
CREATE TABLE IF NOT EXISTS `ssmv`.`pf` (
  ...
  `foto` LONGBLOB NULL,
  PRIMARY KEY (`idpf`),
  INDEX `idquestionario_idx` (`idquestionario` ASC),
  INDEX `idtipoSanguineo_idx` (`idtipoSangue` ASC),
  INDEX `fk_pf_estado_idx` (`idestado` ASC),
  UNIQUE INDEX `cpf_UNIQUE` (`cpf` ASC),
  CONSTRAINT `fk_pf_questionario`
    FOREIGN KEY (`idquestionario`)
    REFERENCES `ssmv`.`questionario` (`idquestionario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `ssmv`.`pj` (
  ...
  `telefoneF` VARCHAR(10) NOT NULL,
  `telefoneF2` VARCHAR(45) NOT NULL,
  `foto` LONGBLOB NULL,
  PRIMARY KEY (`idpj`),
...
CREATE TABLE IF NOT EXISTS `ssmv`.`usuarios` (
  ...
  INDEX `fk_usuarios_pf_idx` (`idpf` ASC),
  INDEX `fk_usuarios_pj_idx` (`idpj` ASC),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC),
  UNIQUE INDEX `idpf_UNIQUE` (`idpf` ASC),
  UNIQUE INDEX `idpj_UNIQUE` (`idpj` ASC),
  UNIQUE INDEX `idfacebook_UNIQUE` (`idfacebook` ASC),
  CONSTRAINT `fk_usuarios_pf`
    FOREIGN KEY (`idpf`)
    REFERENCES `ssmv`.`pf` (`idpf`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_usuarios_pj`
    FOREIGN KEY (`idpj`)
    REFERENCES `ssmv`.`pj` (`idpj`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
